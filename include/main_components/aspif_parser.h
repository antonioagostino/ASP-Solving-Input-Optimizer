...
namespace aspsio {
    ...
    class AspifParser {
        private:
            ...
            std::vector<std::unordered_map<int, AspifLiteral*>> aux_predicates_instances;
        ...
        public:
            AspifParser(std::list<std::string> &input_data, std::vector<std::list<AspifStatement*>> &rules_sets, 
                    std::vector<std::string> &pattern_set);
            virtual void StartAnalysis();
            void AddReverseParsingOption(const std::string &pattern);
            std::vector<std::unordered_map<int, AspifLiteral*>>& GetAuxPredicatesInstances(){
